...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      const rootRect = (rootRef.current as any).getBoundingClientRect();
      ...
      if (originalImg) {
        const img = (originalImg as any).cloneNode();
        img.style.cssText = 'width:100%;height:100%;object-fit:cover;';
        animatingOverlay.appendChild(img);
      }
      overlay.remove();
      (rootRef.current as any).appendChild(animatingOverlay);
  ...
  const openItemFromElement = useCallback(
    (el: any) => {
      ...
      (viewerRef.current as any).appendChild(overlay);
